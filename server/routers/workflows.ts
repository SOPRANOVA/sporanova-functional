...
export const workflowsRouter = router({
  ...
  create: workspaceManagerProcedure.input(workspaceInput.extend({ name: z.string().trim().min(2).max(160), description: z.string().trim().max(4000).optional(), nodes: z.array(nodeInput).min(1).max(50) })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const result = await db.insert(workflows).values({ workspaceId: ctx.workspaceId, name: input.name, description: input.description, createdById: ctx.user.id }); const workflowId = Number(result[0].insertId); await db.insert(workflowNodes).values(input.nodes.map(node => ({ workflowId, ...node }))); await writeAuditLog({ workspaceId: ctx.workspaceId, actorUserId: ctx.user.id, action: "workflow.created", resourceType: "workflow", resourceId: workflowId }); return { id: workflowId }; }),
