...
export async function enqueueJob(input: { workspaceId?: number; type: string; payload: Record<string, unknown>; runAt?: Date; maxAttempts?: number }) {
  ...
  const result = await db.insert(jobs).values({ workspaceId: input.workspaceId, type: input.type, payload: input.payload, runAt: input.runAt ?? new Date(), maxAttempts: input.maxAttempts ?? 3 });
...
export async function claimNextJob(workerId: string) {
  ...
  const claimed = await db.update(jobs).set({ status: "running", attempts: candidate.attempts + 1, lockedAt: new Date(), lockedBy: workerId }).where(and(eq(jobs.id, candidate.id), eq(jobs.status, "pending")));
  if (!claimed[0].affectedRows) return null;
