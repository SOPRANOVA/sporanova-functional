          ...
          <div className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              <Link
                key={item.href}
                to={item.href}
                className="px-4 py-2 text-sm transition-all duration-200 rounded-lg hover:bg-sn-100"
                style={{
      ...
      >
        ...
        <div className="relative h-full flex flex-col pt-20 px-6 gap-2">
          ...
          <div className="flex flex-col gap-3 mt-6">
            ...
            <Link to="/signup" className="py-3 text-center text-sm font-medium rounded-xl text-sn-white" style={{ background: "#1A1F3C" }}>Get Started</Link>
          </div>
        </div>
      </div>
    </>
